...
def security_voice_fingerprint(granted_users: list, outputLabel: QLabel):
    ...
    for user_path in users_records_paths:
        ...
        # similarity_score = torch.max(torch.nn.functional.pairwise_distance(mean_reference_features.unsqueeze(0),
        #                                                         passcode_features.unsqueeze(0).unsqueeze(0)))

        similarity_score = compare_audio_files(mean_reference_features, passcode_features)
    ...
    print(values_torch)
    values_torch = values_torch * 10000
    values_torch = values_torch
    print(values_torch)

    values_torch /= (torch.max(values_torch) + 100)
    ...
    for granted_user in granted_users:
        ...
        if user_score >= threshold:
            ...
            outputLabel.setText(f"Sentence: {sentence} - Access granted, welcome back Mr")
            return True, values_torch.cpu().numpy(), sentence, sentences_probabilities.cpu().numpy()

    print("Access denied")
    outputLabel.setText(f"Sentence: {sentence} - Acces denied, how else can I help you?")
    return False, values_torch.cpu().numpy(), sentence, sentences_probabilities.cpu().numpy()
